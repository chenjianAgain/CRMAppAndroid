...
import com.zendaimoney.android.athena.im.manager.ContacterManager.MRosterGroup;
...
public abstract class AContacterActivity extends ActivitySupport {
	...
	protected void sendSubscribe(Presence.Type type, String to) {
		...
		presence.setTo(to);
		XmppConnectionManager.getInstance(context).getConnection()
				.sendPacket(presence);
	}
	...
	protected void createSubscriber(String userJid, String nickname,
		...
		AppLog.i(TAG, "-------------createSubscriber--------------userJid="+userJid+"----nickname="+nickname+"-----groups="+groups);
		
		XmppConnectionManager.getInstance(context).getConnection().getRoster()
